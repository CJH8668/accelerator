...
//       
//       Creation Date:  Wed Jun 23 13:48:43 2021
//       Version: 	r0p1
//
...
`ifdef POWER_PINS
module itcm_ram (VDDCE, VDDPE, VSSE, Q, CLK, CEN, WEN, A, D, EMA, EMAW, EMAS, GWEN,
    RET1N);
`else
module itcm_ram (Q, CLK, CEN, WEN, A, D, EMA, EMAW, EMAS, GWEN, RET1N);
`endif
 ...
 wire  CLK_;
  ...
  assign RET1N_ = RET1N;
   ...
   `ifdef ARM_FAULT_MODELING
     itcm_ram_error_injection u1(.CLK(CLK_), .Q_out(Q_), .A(A_int), .CEN(CEN_int), .GWEN(GWEN_int), .WEN(WEN_int), .Q_in(Q_int));
  `else
...
`endif
  ...
  reg NOT_WEN6, NOT_WEN5, NOT_WEN4, NOT_WEN3, NOT_WEN2, NOT_WEN1, NOT_WEN0, NOT_A12;
  reg NOT_A11, NOT_A10, NOT_A9, NOT_A8, NOT_A7, NOT_A6, NOT_A5, NOT_A4, NOT_A3, NOT_A2;
 ...
 wire  CLK_;
  ...
  buf B205(D_[62], D[62]);
  buf B206(D_[63], D[63]);
  buf B207(EMA_[0], EMA[0]);
  buf B208(EMA_[1], EMA[1]);
  buf B209(EMA_[2], EMA[2]);
  buf B210(EMAW_[0], EMAW[0]);
  buf B211(EMAW_[1], EMAW[1]);
  buf B212(EMAS_, EMAS);
  buf B213(GWEN_, GWEN);
  buf B214(RET1N_, RET1N);
...
`endif
`timescale 1ns/1ps
module itcm_ram_error_injection (Q_out, Q_in, CLK, A, CEN, WEN, GWEN);
   output [63:0] Q_out;
   input [63:0] Q_in;
   input CLK;
   input [12:0] A;
